...
## Importing results
```{r}
results <- list.files(path = "../Analysis NR2F2 mean intensity/", recursive = TRUE, pattern = ".csv$", full.names = TRUE)
...
E14_rep1 <- df_channel_NR2F2_rep1 %>% filter(Line == "E14")

E14_rep1_mean <- mean(E14_rep1$Mean)

E14_rep2 <- df_channel_NR2F2_rep2 %>% filter(Line == "E14")

E14_rep2_mean <- mean(E14_rep2$Mean)


df_channel_NR2F2_rep1_norm <- df_channel_NR2F2_rep1 %>% mutate(Norm_Mean = df_channel_NR2F2_rep1$Mean/E14_rep1_mean)

df_channel_NR2F2_rep2_norm <- df_channel_NR2F2_rep2 %>% mutate(Norm_Mean = df_channel_NR2F2_rep2$Mean/E14_rep2_mean)
...
plot_fun <- function(data, var, label_nudge = 5e4) {
  ...
  scale_y_continuous(
  limits = c(0, 2), expand = c(0, 0, 0.1, 0)
    ) +
    scale_fill_brewer(
      palette = "Set1",
      guide  = "none"
...
```{r}
p1 <- plot_fun(df_channel_NR2F2_rep1_norm, Mean) +
  labs(x = "Line", y = "NR2F2 norm. mean intensity Rep.1")


p2 <- plot_fun(df_channel_NR2F2_rep2_norm, Mean) +
  labs(x = "Line", y = "NR2F2 norm. mean intensity Rep.2")

p <- plot_fun(norm_df_df_channel_NR2F2, Norm_Mean) +
  labs(x = "Line", y = "NR2F2 mean intensity")
...
ggsave(
  "MSGN1KO_NR2F2_mean.svg", # <- change this
  plot   = p, 
