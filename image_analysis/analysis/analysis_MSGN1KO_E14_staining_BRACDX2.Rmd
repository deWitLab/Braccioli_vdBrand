...
## NOTE: BRA = TBXT
```{r}
results <- list.files(path = "../Analysis CDX2_BRA mean intensity/", recursive = TRUE, pattern = ".csv$", full.names = TRUE)
...
df_channel_CDX2 <- filter(df_channel, grepl(staining_name, area_label))

df_channel_CDX2_rep1 <- filter(df_channel_CDX2, Replicate == "REP1")

df_channel_CDX2_rep2 <- filter(df_channel_CDX2, Replicate == "REP2")


E14_rep1 <- df_channel_CDX2_rep1 %>% filter(Line == "E14")

E14_rep1_mean <- mean(E14_rep1$Mean)

E14_rep2 <- df_channel_CDX2_rep2 %>% filter(Line == "E14")

E14_rep2_mean <- mean(E14_rep2$Mean)


df_channel_CDX2_rep1_norm <- df_channel_CDX2_rep1 %>% mutate(Norm_Mean = df_channel_CDX2_rep1$Mean/E14_rep1_mean)

df_channel_CDX2_rep2_norm <- df_channel_CDX2_rep2 %>% mutate(Norm_Mean = df_channel_CDX2_rep2$Mean/E14_rep2_mean)

norm_df_df_channel_CDX2 <- rbind(df_channel_CDX2_rep1_norm, df_channel_CDX2_rep2_norm)


```

## Analysis for channel 2 - BRA
...
df_channel_BRA <- filter(df_channel, grepl(staining_name, area_label))

df_channel_BRA_rep1 <- filter(df_channel_BRA, Replicate == "REP1")

df_channel_BRA_rep2 <- filter(df_channel_BRA, Replicate == "REP2")

E14_rep1 <- df_channel_BRA_rep1 %>% filter(Line == "E14")

E14_rep1_mean <- mean(E14_rep1$Mean)

E14_rep2 <- df_channel_BRA_rep2 %>% filter(Line == "E14")

E14_rep2_mean <- mean(E14_rep2$Mean)

df_channel_BRA_rep1_norm <- df_channel_BRA_rep1 %>% mutate(Norm_Mean = df_channel_BRA_rep1$Mean/E14_rep1_mean)

df_channel_BRA_rep2_norm <- df_channel_BRA_rep2 %>% mutate(Norm_Mean = df_channel_BRA_rep2$Mean/E14_rep2_mean)

norm_df_df_channel_BRA <- rbind(df_channel_BRA_rep1_norm, df_channel_BRA_rep2_norm)
...
plot_fun <- function(data, var, label_nudge = 5e4) {
  ...
  scale_y_continuous(
  limits = c(0, 10), expand = c(0, 0, 0.1, 0)
    ) +
    scale_fill_brewer(
      palette = "Set1",
      guide  = "none"
...
# Plotting area and mean intensity CDX2

```{r}
p1 <- plot_fun(df_channel_CDX2, Mean) +
  labs(x = "Line", y = "CDX2 norm. mean intensity Rep.1")


p2 <- plot_fun(df_channel_CDX2_rep2_norm, Mean) +
  labs(x = "Line", y = "CDX2 norm. mean intensity Rep.2")

p <- plot_fun(norm_df_df_channel_CDX2, Norm_Mean) +
  labs(x = "Line", y = "CDX2 mean intensity")
...
# Plotting area and mean intensity BRA

```{r}
p1 <- plot_fun(df_channel_BRA, Mean) +
  labs(x = "Line", y = "BRA mean intensity") +
 facet_wrap(~Replicate) 


p2 <- plot_fun(df_channel_BRA, IntDen) +
  labs(x = "Line", y = "BRA int dens") +
  facet_wrap(~Replicate)

p3 <- plot_fun(norm_df_df_channel_BRA, Norm_Mean) +
  labs(x = "Line", y = "BRA mean intensity")
...
E14 <- norm_df_df_channel_BRA %>% filter(Line == "E14")

C3_7 <- norm_df_df_channel_BRA %>% filter(Line == "3-7")

C4_5 <- norm_df_df_channel_BRA %>% filter(Line == "4-5")

E14_C3_7 <- norm_df_df_channel_BRA %>% filter(Line == "E14+3-7")

E14_C4_5 <- norm_df_df_channel_BRA %>% filter(Line == "E14+4-5")
...
ggsave(
  "MSGN1KO_CDX2_mean.svg", # <- change this
  plot   = p, 
