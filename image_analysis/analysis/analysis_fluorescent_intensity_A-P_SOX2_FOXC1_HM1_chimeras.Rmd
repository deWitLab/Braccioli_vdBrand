...
## Importing results
```{r}
results <- list.files(path = "../chimeras_HM1/", recursive = TRUE, pattern = ".csv$", full.names = TRUE)
...
chimeras <- df %>% filter(df$line == c("E14+MSGN1KO"))

single <- df %>% filter(df$line == c("E14", "MSGN1KO"))
