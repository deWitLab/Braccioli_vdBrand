...
path_in <- normalizePath("../../processed_data/sciatac/fastq_namepaste/timecourse2/")
path_out <- normalizePath("../../processed_data/sciatac/bam/timecourse2/")
...
command <- paste0("bwa mem -MC -t 30 ",
...
edit_tags <- paste0("samtools view -h ", bamfile, " | ",
                    ...
                    path_out, "/timecourse2_tagedit.bam")
edit_tags <- gsub("\\\\", "", edit_tags, perl = TRUE)

system(command)
system(paste0("samtools index ", bamfile))
system(edit_tags)
system(paste0("samtools index ", path_out, "/timecourse2_tagedit.bam"))