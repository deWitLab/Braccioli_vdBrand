...
moi <- c("SOX2", "POU5F1", "TBX6", "TCF3", 
         "TWIST1", "GATA6", "TCF7", "ETV1",
         "RFX4", "FOXD2", "Zic2", "TBXT")
moi <- setNames(match(moi, meta$name), replace(moi, c(2, 11), c("OCT4", "ZIC2")))
...
group1 <- c("SOX2", "RFX4", "TCF3", "TCF7")
p1 <- ggplot(subset(df, motif %in% group1), aes(x, y)) +
  ggrastr::rasterise(
    geom_outline_point(
      aes(colour = value,
          stroke_colour = after_scale(edge_colour(colour))),
  ...
  geom_polygon(
    data = subset(logos, motif %in% group1),
    aes(x, y, fill = I(fill), group = group)
  ) +
  scale_colour_gradientn(
    colours = div_colours, limits = ~ c(-4, 4),
...
group2 <- c("OCT4", "TBX6", "TWIST1", "FOXD2")
p2 <- ggplot(subset(df, motif %in% group2), aes(x, y)) +
  ggrastr::rasterise(
    geom_outline_point(
      aes(colour = value,
          stroke_colour = after_scale(edge_colour(colour))),
  ...
  geom_polygon(
    data = subset(logos, motif %in% group2),
    aes(x, y, fill = I(fill), group = group)
  ) +
  scale_colour_gradientn(
    colours = div_colours, limits = ~ c(-4, 4),
...
group3 <- setdiff(names(moi), c(group1, group2))
p3 <- ggplot(subset(df, motif %in% group3), aes(x, y)) +
  ggrastr::rasterise(
    geom_outline_point(
      aes(colour = value,
          stroke_colour = after_scale(edge_colour(colour))),
  ...
  geom_polygon(
    data = subset(logos, motif %in% group3),
    aes(x, y, fill = I(fill), group = group)
  ) +
  scale_colour_gradientn(
    colours = div_colours, limits = ~ c(-4, 4),
...
```{r}
ggsave(
  here("figures", "manuscript", "FigS3_Extra_UMAP_Motifs_1.svg"), plot = p1,
  device = svglite::svglite, width = 170, height = 100, unit = "mm",
  fix_text_size = FALSE
)
ggsave(
  here("figures", "manuscript", "FigS3_Extra_UMAP_Motifs_2.svg"), plot = p2,
  device = svglite::svglite, width = 170, height = 100, unit = "mm",
  fix_text_size = FALSE
)
ggsave(
  here("figures", "manuscript", "FigS3_Extra_UMAP_Motifs_3.svg"), plot = p3,
  device = svglite::svglite, width = 170, height = 100, unit = "mm",
  fix_text_size = FALSE
)
```
...
acf_score <- function(x) {
  n <- length(x)
  acf <- acf(x, n - 1, plot = FALSE)$acf
  sum(abs(acf) > (1.96 / sqrt(n - 1))) / n
}

scores <- apply(zscore, 1, acf_score)
zscore <- zscore[scores > 0.5, ]

RhpcBLASctl::blas_set_num_threads(1)

smooth <- apply(zscore, 1, function(x) {
  loess.smooth(seq_along(x), x, evaluation = 100, span = 0.25)$y
})

highlight <- c("SOX2", "CDX2", "RFX4", "Pou5f1::Sox2", "TBXT")
highlight <- setNames(match(highlight, rowData(motifs)$name),
                      highlight)
highlight <- setNames(rownames(motifs)[highlight], names(highlight))

df <- reshape2::melt(smooth)
df$label <- match(df$Var2, highlight)
df$label <- ifelse(is.na(df$label), NA, names(highlight)[df$label])

colours <- c(CYRUP[1:3], "#23AA17", "purple")
names(colours)[1:5] <- names(highlight)

last <- df[df$Var1 == max(df$Var1) & !is.na(df$label), ]
ord <- match(as.character(last$Var2), highlight)
last$label <- names(highlight)[ord]
last$colours <- colours[last$label]
last <- last[last$label != "Pou5f1::Sox2",]


annotate <- data.frame(
  x = scales::rescale(seq_len(ncol(diff)) / ncol(diff), c(1, 100)),
  y = -Inf,
  fill = diff$clusters$cluster
)
d <- median(diff(annotate$x))
range <- c(-1, 1) * max(abs(df$value))

trends <- ggplot(df, aes(Var1, value)) +
  geom_line(aes(group = Var2),
            data = ~ subset(.x, is.na(label)),
            alpha = 0.3, size  = 0.1) +
  geom_line(aes(group = Var2, colour = label),
            data = ~ subset(.x, !is.na(label))) +
  geom_rect(
    aes(xmin = x -d, xmax = x + d, 
        ymin = -Inf, ymax = min(range), fill = fill),
    data = annotate, inherit.aes = FALSE, colour = NA
  ) +
  scale_y_continuous(
    limits = function(x) {c(-1, 1) * max(abs(x))},
    name = "Motif Z-score",
    labels = function(x) {
      ifelse(sign(x) == 1, paste0("+", x),
             ifelse(sign(x) == -1, gsub("-", "\u2212", as.character(x)), x))
    }
  ) +
  scale_x_continuous(
    labels = number_format(scale = 1e-2, accuracy = 0.1),
    name = "Pseudotime Quantile", expand = c(0,0),
    breaks = c(1, 50.5, 100),
    limits = c(1 -d, 100 + d)
  ) +
  scale_colour_manual(
    values = colours, na.value = colours[5],
    guide = "none"
  ) +
  scale_fill_manual(
    # values = c(A = "#fed500", B = "#4DAF4A", C = "#A65628", D = "#984EA3", 
    #            E = "#FF7F00", F = "#377EB8", G = "#F781BF", H = "#999999"),
    values = clust_colour,
    guide = "none"
  ) +
  guides(y.sec = guide_axis_manual(
    breaks = last$value, labels = last$label,
    label_colour = darken(last$colours, 0.2)
  )) +
  annotate(
    geom = "text", x = 25, y = 2.5, label = "OCT4::SOX2",
    colour = darken("#23AA17", 0.2), size = 6/.pt
  ) +
  theme(aspect.ratio = NULL,
...
```{r}
ggsave(
  here("figures", "manuscript", "Fig3_Pseudotime_trends.svg"), plot = trends,
...
```{r}
ggsave(
  here("figures", "manuscript", "Fig3_Heatmap_Neuro_arm.svg"), plot = heatmap,
  device = svglite::svglite, width = 86, height = 86, unit = "mm",
  fix_text_size = FALSE
)
...
```{r}
diff <- exp[, exp$clusters$cluster %in% c("A", "D", "E", "F")]
...
heatmap2 <- ggplot(cbind(df2, facet = facet_vars[1]), aes(Var2, Var1)) +
  ...
  GENOVA::scale_fill_GENOVA_div(
    ...
    #   barheight = unit(nlevels(df$Var1) * 8.4, "pt")
    # )
  ) +
  ggnewscale::new_scale_fill() +
  geom_raster(
...
svglite::svglite(
  here("figures", "manuscript", "Fig3_Heatmap_Combined.svg"),
  width = convertUnit(unit(170, "mm"), "in", valueOnly = TRUE),
